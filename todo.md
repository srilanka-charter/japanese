...
- [ ] アクティビティページ（/activity）を新規作成（6セクション：サファリ・アーユルヴェーダ・ビレッジツアー・エレファントライド・ホエールウォッチング・占星術）
- [ ] ヘッダーに「アクティビティ」メニューを価格の横に追加
- [ ] sitemap.xmlに/activityを追加
- [ ] ホエールウォッチング用画像（シロナガス鯨の尾）を生成
- [ ] ビレッジツアー用画像（牛車+料理体験の合成）を生成
- [ ] 占星術セクション用画像（先生の写真+紹介テキスト合成）を生成
