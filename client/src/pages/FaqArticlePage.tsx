...
const faqAnswers: Record<string, string> = {
  ...
  "日本語で旅程の相談やルート提案をしてもらえますか？":
    "はい、可能です。ご希望の行程・訪問地をお伝えいただければ、現地事情を踏まえた移動ルート案・所要時間の目安をご案内します（参考情報の範囲）。旅程の最終決定はお客様ご自身が行います。",
  "サファリやホエールウォッチングなどのアクティビティ手配はお願いできますか？":
    "シルバープラン以上をご利用の場合、サファリやホエールウォッチングの手配についてはドライバー経由でのご依頼が可能です（現地実費）。予約・支払い・契約はお客様とドライバーの間で直接行ってください。",
  "ドライバーの運転技術や時間厳守は信頼できますか？":
    "当社は資格・実績などの基準に基づいてドライバーを選定し、日本人ゲストが重視するポイントを事前に共有しています。ただし、当社は運送品質を保証するものではありません。",
  "ベビーシートやチャイルドシートは用意してもらえますか？":
    "可能な範囲でご案内いたします（在庫・地域によって異なります）。ご利用をご希望の場合は、事前にお問い合わせください。",
  "ドライバーとの合流はスムーズにできますか？":
    "出発前にドライバーの連絡先をお伝えしますので、事前にコンタクトが可能です。万一の事態が発生した場合も、メールや連絡窓口を通じてサポートいたします。",
  "観光地でのガイドや解説もしてもらえますか？":
    "シルバープランではTourist Driver以上、ゴールドプランではChauffeur Guide Driverがアテンドします。シルバープラン以上であれば観光地での同行・解説に対応します。ただ、繁忙期でシルバープランにてTourist Driverがアテンドする場合は、シーギリヤロック・アヌラーダプラ・ポロンナルワはドライバーが同行することができません。この場合は観光地の現地で追加費用なしで現地ガイドを手配させていただきます。また、ドライバーは専門ガイドとは異なりますので、より詳細な解説が必要な場合はお客様ご自身で専門ガイドを手配いただくことをおすすめします。",
  "お支払い方法について教えてください":
    "当社へのお支払い（サービス料・予約取次手数料）はオンラインでのカード決済となります。ドライバー運賃は全額、現地でドライバーへ直接お支払いください。",
  "料金に含まれるものと含まれないものを教えてください":
    "料金に含まれるもの：日本製車両の車代、ドライバーの人件費・食事代・宿泊代、車両保険料、高速道路料金・駐車料金。料金に含まれないもの：チップ（任意）、観光地の入場料、サファリ・アクティビティ等の料金。",
  "紅茶列車の予約ができなかった場合、手配してもらえますか？":
    "鉄道の予約はお客様ご自身でのオンライン手配をお願いしております。予約サイトは「12Go.Asia」のご利用がおすすめです。万が一予約が取れなかった場合でも、立ち席でも楽しめる区間の目安など、移動の観点から参考情報をご案内することは可能です。",
  "直前で旅程や予定が変更になった場合はどうなりますか？":
    "旅行開始の7日前まではキャンセル料0円でサービス料を返金します。2〜6日前はサービス料の返金はありません。前日以降・当日はいかなる理由でも返金不可となります。",
  "初日の到着が深夜になる場合でも移動できますか？":
    "基本的には対応可能ですが、ドライバーの稼働状況・安全面・道路状況等によっては難しい場合もあります。深夜到着の移動をご希望の場合は、事前にご相談ください。",
  "ドライバーのご紹介について教えてください":
    "シルバープラン以上の場合、SLTDA（スリランカ観光開発庁）登録の観光ドライバーをご紹介します。日本語対応ドライバーのご希望には可能な範囲で対応しますが、供給状況等により確約できない場合があります。",
  "海外旅行保険への加入は必要ですか？":
    "加入を強くおすすめします。スリランカでは法令により全車両への保険加入が義務付けられていますが、補償水準は低く、死亡事故でも補償金は日本円換算で約30万円程度にとどまります。お客様ご自身による海外旅行保険への加入が最も確実な手段です。",
};
